...
void Text::AddLine(const Ref<Font>& font, const std::string& text, const uint2& position, const float4& colour, const float4& backgroundColour)
{
...
void Text::Update(const Transform& transform)
{
	if (CreateInfoHasChanged(&m_CI))
	{
		...
		if (!m_Camera)
		{
			m_CameraCI.debugName = "GEAR_CORE_Text";
...
void Text::GenerateLine(size_t lineIndex, bool update)
{
	...
	fontMaterialCI.debugName = "GEAR_CORE_Text: " + font->textureAtlas->GetCreateInfo().debugName;
	...
	{
		Font::GlyphInfo gi = font->glyphInfos[static_cast<size_t>(*it)];
	...
	textMeshCI.modelData = CreateRef<ModelData>(modelData);
	if (!update || line.model == nullptr)
	{
		Model::CreateInfo textModelCI;
		textModelCI.debugName = "GEAR_CORE_Text: " + line.text;
