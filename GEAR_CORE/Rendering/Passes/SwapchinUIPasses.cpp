...
void SwapchinUIPasses::ExternalUI(Renderer& renderer, ui::UIContext* uiContext, Renderer::PFN_SetPassParameters pfnSetPassParameters, Renderer::PFN_RenderDrawData pfnRenderDrawData)
{
	RenderGraph& renderGraph = renderer.GetRenderGraph();
	const Ref<RenderSurface>& renderSurface = renderer.GetRenderSurface();
	const Ref<Window>& window = renderer.GetWindow();
	...
	if (uiContext && pfnSetPassParameters)
	{
		(uiContext->*pfnSetPassParameters)(externalUIPassParameters);
	}
	...
	renderGraph.AddPass("External UI", externalUIPassParameters, CommandPool::QueueType::GRAPHICS,
		[uiContext, pfnRenderDrawData](Ref<CommandBuffer>& cmdBuffer, uint32_t frameIndex)
		{
			if (uiContext && pfnRenderDrawData)
			{
				(uiContext->*pfnRenderDrawData)(cmdBuffer, frameIndex);
			}
		});
}