		...
		if (ImGui::BeginDragDropTarget())
		{
			...
			if (payload)
			{
				...
				if (std::filesystem::exists(filepath))
				{
					...
					{
						Ref<asset::manager::EditorAssetManager> editorAssetManager = UIContext::GetUIContext()->GetEditorAssetManager();
...
void ViewportPanel::UpdateCameraTransform()
{
	...
	if (ImGui::IsWindowFocused() && ImGui::IsWindowHovered())
	{
		...
		if (ImGui::IsMouseDown(ImGuiMouseButton_::ImGuiMouseButton_Left) && !m_GuizmoActive)
		{
			...
			Ref<Model> nearestModel = objects::Picker::GetNearestModel(m_CI.renderer->GetModelQueue(), camera, mouse, m_CurrentSize);
			if (nearestModel)
			{
			...
			Ref<Light> nearestLight = objects::Picker::GetNearestLight(m_CI.renderer->GetLights(), camera, mouse, m_CurrentSize);
			if (nearestLight)
			{
