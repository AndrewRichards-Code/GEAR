...
void UIContext::SetPassParameters(Ref<graphics::rendering::TaskPassParameters>& passParameters)
{
	using namespace graphics::rendering;

	for (const auto& textureID : m_TextureIDs_PF)
	{
		passParameters->AddResourceViewAtPipelineStage(ResourceView(textureID.first, Resource::State::SHADER_READ_ONLY), PipelineStageBit::FRAGMENT_SHADER_BIT);
	}
	m_TextureIDs_PF.clear();
}
