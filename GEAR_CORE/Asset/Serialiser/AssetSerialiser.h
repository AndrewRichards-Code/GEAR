...
namespace gear
{
	namespace asset
	{
		...
		{
			class GEAR_ASSET_MANAGER_API AssetSerialiser
			{
				//Methods
			public:
				...
				static void SetAssetManager(const Ref<manager::AssetManager>& assetManager);
				static Ref<manager::AssetManager> GetAssetManager();
			...
			private:
				...
				static inline Ref<manager::AssetManager> s_AssetManager = nullptr;
			};
			...
			inline Ref<T> ToAsset(const YAML::Node& node)
			{
				...
				const Ref<manager::AssetManager>& assetManager = AssetSerialiser::GetAssetManager();
