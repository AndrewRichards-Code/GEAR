...
Ref<Asset> ModelLoader::Deserialise(Asset::Handle handle, const AssetMetadata& metadata)
{
	Ref<ModelData> meshData = CreateRef<ModelData>(LoadModelData(metadata.filepath));
	meshData->handle = handle;
	return meshData;
}

void ModelLoader::Serialise(Ref<Asset> asset, const AssetMetadata& metadata)
{
	GEAR_WARN(ErrorCode::ASSET | ErrorCode::NOT_SUPPORTED, "No support for Serialising models.");
}
...
ModelData ModelLoader::LoadModelData(const std::filesystem::path& filepath)
{
...
void ModelLoader::BuildNodeGraph(const aiScene* scene, aiNode* node, MeshNode& thisNode, ModelData& modelData)
{
...
std::vector<MeshData> ModelLoader::ProcessMeshes(aiNode* node, const aiScene* scene)
{
