...
gear::Ref<RenderPipeline> ImageProcessing::s_PipelineMipMaps;
...
void ImageProcessing::GenerateMipMaps(Texture* texture)
{
	...
	{ 
		...
		s_PipelineMipMaps = CreateRef<RenderPipeline>(&s_PipelineLI);
	}
	...
	m_DescSetCI.pDescriptorSetLayouts = s_PipelineMipMaps->GetDescriptorSetLayouts();
	...
	//Record Compute CommandBuffer
	{
		...
		m_ComputeCmdBuffer->BindPipeline(0, s_PipelineMipMaps->GetPipeline());
		for (uint32_t i = 1; i < levels; i++)
		{
			...
			preDispatch.subresoureRange = { Image::AspectBit::COLOUR_BIT, i, 1, 0, 1 };
			...
			m_ComputeCmdBuffer->BindDescriptorSets(0, { m_DescSets[i - 1] }, s_PipelineMipMaps->GetPipeline());
			...
			postDispatch.subresoureRange = { Image::AspectBit::COLOUR_BIT, i, 1, 0, 1 };
