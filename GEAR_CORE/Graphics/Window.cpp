...
bool Window::Init()
{
	...
	{
		GLFWimage icon;
		icon.pixels = m_CI.iconData->Data.data();
		icon.width = m_CI.iconData->width;
		icon.height = m_CI.iconData->height;
...
void Window::UpdateWindowCreateInfo(const core::ConfigFile& configFile, Window::CreateInfo& windowCI)
{
	windowCI.width					= configFile.GetOption<uint32_t>("windowedWidth");
	windowCI.height					= configFile.GetOption<uint32_t>("windowedHeight");
	windowCI.fullscreen				= configFile.GetOption<bool>("fullscreen");
	windowCI.fullscreenMonitorIndex = configFile.GetOption<uint32_t>("fullscreenMonitorIndex");
	windowCI.maximised				= configFile.GetOption<bool>("maximised");
}
